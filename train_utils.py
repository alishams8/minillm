...
class TrainerUtils:
    ...
    def __init__(self):
        ...
        self.loss_fn = nn.CrossEntropyLoss(ignore_index=self.tokenizer.token_to_id('[PAD]')).to("cpu")
    ...
    def train_one_step(self, model_inp: dict, model: nn.Module, optimizer:torch.optim.Adam):
        ...
        src_x = model_inp["encoder_input_ids"].to("cpu")
        src_attn_mask = model_inp["encoder_attention_mask"].to("cpu")
        tgt_x = model_inp["decoder_input_ids"].to("cpu")
        tgt_attn_mask = model_inp["decoder_attention_mask"].to("cpu")
        labels = model_inp["labels"].to("cpu")
